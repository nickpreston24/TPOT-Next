...
import { InputLeftElement, InputRightElement } from '@chakra-ui/core/dist/InputElement'
            ...
            <form onSubmit={formik.handleSubmit}>
              {['forgot'].includes(mode) && (
                <Box mb={6} color='white'>
                  We will send you an email to reset your password. Please check your spam folder!
                </Box>
              )}
              <TextField
                name='firstName'
              ...
              <TextField
                name='email'
                type='email'
                enabledModes={['login', 'register', 'forgot']}
                {...{ mode, formik }}
              />
              <TextField
                name='password'
...
const TextField = ({ name, type, enabledModes, mode, formik }) => {
  ...
  if (enabled) {
    children = (
      <FormControl isInvalid={inputError} width='100%'>
        ...
        <InputGroup>
          <InputLeftElement
            children={<Icon name={formIcons[name]} fontSize='md' color='gray.300' />}
          />
          ...
          <InputRightElement
            children={
              <>
                {isPassword ? (
                  <Button onClick={togglePassword} variant='ghost'>
                    <Icon name={showicon} fontSize='lg' color='primary.600' />
                  </Button>
                ) : (
  ...
  return (
    <Box
      h={!enabled ? 0 : inputError ? 100 : 80}
      transition={inputError && 'all 0.2s ease-out'}
      width='100%'
    >
      {children}
    </Box>
